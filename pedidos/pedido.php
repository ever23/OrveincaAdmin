...
{
	$COTIZACION=$database->insertar_pedido($_GET['idet_clie'],$_GET['ci_empl'],$_GET['nped_manu']);
}
else
{
	$html->__destruct();
...
$(document).ready(function(e) {
     $('#generate_pedi').click(function(e) {
	
		e.preventDefault();
       var href=  $(this).attr('href');
	  location.href=href+'&ci_empl='+$('select[name=ci_empl]').attr('value')+'&nped_manu='+$('input[name=nped_manu]').attr('value');
    });
});
</script>
 <div align="center" class="conten_ico" >
 <A href="<?php echo $_SERVER['HTTP_REFERER'] ?>"><div class="atras"></div></A>
 <a href="<?php echo $_SERVER['PHP_SELF']."?pdf=true&idet_clie=".$cliente['idet_clie']; ?>" id="generate_pedi">
<div class="save" >
</div></a>
	 ...
	 while($telefono=$database->result())
	{
		...
		?>
            ...
            <tr class="col_hov ">
              ...
              <td><input name="nped_manu" type="text" ></td>
            </tr>
          </table>
...
}elseif(!empty($_GET['pdf']) && !empty($_GET['idet_clie']))
{
	if(!$database->error())
	{
			?>
<script>
var src='<?php echo "pedido_pdf.php?nume_pedi=".$COTIZACION ?>';
$(document).ready(function(e) {
star_load_pdf();
...
<?PHP echo "<div align='center' id='pag_pdf'><a href='' id='carta'><div>CARTA</div></a>  <a href='' id='oficio'><div>OFICIO</div></a></div>
  ...
  <iframe width='960' height='600' id='nota_pdf' src='pedido_pdf.php?nume_pedi=".$COTIZACION."'></iframe>";
